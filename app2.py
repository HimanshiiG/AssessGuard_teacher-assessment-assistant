...
def display_questions(questions):
    ...
    if st.button("Submit"):
        insert_question_answer_pairs_query(questions, answers, 0)
    # Display the user responses
    # st.subheader("Your Responses:")
    # font_size = 24
    # for i, response in enumerate(user_responses, start=1):
    #     st.markdown(f"<p style='font-size: {font_size}px;'>Question {i}: {response}</p>", unsafe_allow_html=True)
        # st.markdown(f"<p style='font-size: 32px;'>Question {i}: {response} </p>")
